...
If tIndex > 0 Then
    If MsgBox("Seguro queres hacer admin a " & cboPjs.Text & "?", vbYesNo) = vbYes Then
...
adminpasswd = IIf(Len(svrname.Text) > 2, adminpas.Text, "CONTRASEÃ‘A")
If adminpasswd = "CONTRASEÃ‘A" Then
txStatus.Caption = "INGRESE UNA CONTRASEÃ‘A!!"
Else
txStatus.Caption = ""
End If
...
Exit Sub
End If
    If MsgBox("Seguro queres echar a " & cboPjs.Text & " del servidor?!", vbYesNo) = vbYes Then
    ...
    vivociu = UserVivos(eKip.eCUI) > 0
        ...
        If (vivopk = False And numpk > 0) Or (vivociu = False And numciu > 0) Or ((vivopk = False And numpk > 0) And (vivociu = False And numciu > 0)) Then
            ...
            If vivociu = True Then
                winciu = winciu + 1
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("EL EQUIPO ROJO GANO LA RONDA!.", FontTypeNames.FONTTYPE_TALK))
                
            ElseIf vivopk = True Then
                winpk = winpk + 1
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("EL EQUIPO AZUL GANO LA RONDA!.", FontTypeNames.FONTTYPE_TALK))
            End If
...
Private Sub wssvr_DataArrival(ByVal bytesTotal As Long)
    ...
    Debug.Print msg & "asd"
        ...
        wssvr.SendData "@|ç" & wssvr.LocalIP & "ç" & Puerto & "ç" & svname & "ç" & mapax.list(servermap - 1) & "ç" & NumUsers
    End If
End Sub
