...
'17/09/02
'Agregue que la funciçn se asegure que el objeto no es un barco
...
'[Barrin 17-12-03] Si el usuario dejç de ser Newbie, y estaba en el Newbie Dungeon
...
'çel user ya tiene un objeto del mismo tipo?
Slot = 1
...
If MapData(UserList(UserIndex).Pos.map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex > 0 Then
    'çEsta permitido agarrar este obj?
    If ObjData(MapData(UserList(UserIndex).Pos.map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex).Agarrable <> 1 Then
        ...
        Else
            ...
            If ObjData(MiObj.ObjIndex).Log = 1 Then
                Call LogDesarrollo(UserList(UserIndex).name & " juntç del piso " & MiObj.amount & " " & ObjData(MiObj.ObjIndex).name)
...
'24/01/2007 Pablo (ToxicWaste) - Agrego el Cuerno de la Armada y la Legiçn.
'24/01/2007 Pablo (ToxicWaste) - Utilizaciçn nueva de Barco en lvl 20 por clase Pirata y Pescador.
'
...
Select Case Obj.OBJType
    Case eOBJType.otWeapon
        If UserList(UserIndex).flags.Muerto = 1 Then
            'Call WriteConsoleMsg(UserIndex, "ççEstas muerto!! Solo podes usar items cuando estas vivo. ", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
    ...
    Case eOBJType.otPociones
        ...
        If Not IntervaloPermiteAtacar(UserIndex, False) Then
            Call WriteConsoleMsg(UserIndex, "ççDebes esperar unos momentos para tomar otra pocion!!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
    ...
    Case eOBJType.otBarcos
        ...
        Else
            Call WriteConsoleMsg(UserIndex, "çDebes aproximarte al agua para usar el barco!", FontTypeNames.FONTTYPE_INFO)
        End If
End Select
    ...
    For i = 1 To MAX_INVENTORY_SLOTS
        ...
        If ItemIndex > 0 Then
             If ItemSeCae(ItemIndex) Then
                ...
                'Si es pirata y usa un Galeçn entonces no explota los items. (en el agua)
